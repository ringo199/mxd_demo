...
struct character
{
    ...
    character(int _id, int _master_area_id, int _small_area_id, int _user_id, const char* _name,
        int _level, int _hp, int _mp, int _exp)
    {
        id = _id;
        masterAreaId = _master_area_id;
...
struct gameMap
{
    ...
    gameMap(int _id, object_area _a, img_config _background,
        block* _blockList, enemy _ene, npc* _nnpc, transpoint* _transp)
    {
        id = _id;
        a = _a;
...
container<area> bigAreaList("C:/code/tcp/data/bigArea.json");
container<sarea> smallAreaList("C:/code/tcp/data/smallArea.json");
container<character> characterList("C:/code/tcp/data/character.json");
container<userInfo> userInfoList("C:/code/tcp/data/userInfo.json");
container<lv_data_cfg> lvDataCfgList("C:/code/tcp/data/config/lvDataCfg.json");
container<gameMap> gameMapList("C:/code/tcp/data/gameMap.json");
...
void loadCharacterList()
{
    ...
    int rows_size = cJSON_GetArraySize(data);
    cJSON* row, * id, * masterAreaId, * smallAreaId, * userId, * name, * level, * hp, * mp, * exp;
...
void loadGameMapList()
{
    ...
    for (int i = 0; i < rows_size; ++i)
    {
        ...
        enemy ene = { enemy_size->valueint, nullptr };
